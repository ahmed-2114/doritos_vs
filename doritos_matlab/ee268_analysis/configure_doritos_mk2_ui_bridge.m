...
eeHome = loaded.results.context.ee_home(:);
...
set_model_callbacks(modelName, projectRoot, analysisRoot, eeHome);
add_ui_source_path(subsystemPath, signalBlock, fcnBlock);
update_ik_function_block(fcnBlock, toolOffset);
set_switches_to_ui_source(subsystemPath);

save_system(modelName);
write_report(reportFile, modelName, subsystemPath);
bdclose(modelName);

function set_model_callbacks(modelName, projectRoot, analysisRoot, eeHome)
callbackText = sprintf([ ...
    ...
    'if ~exist(''x_ref_ts'', ''var'') || ~exist(''y_ref_ts'', ''var'') || ~exist(''z_ref_ts'', ''var'')\n' ...
    '    t_home_ref = [0; 0.5];\n' ...
    '    ee_home_ref = [%.9f; %.9f; %.9f];\n' ...
    '    x_ref_ts = timeseries([ee_home_ref(1); ee_home_ref(1)], t_home_ref);\n' ...
    '    y_ref_ts = timeseries([ee_home_ref(2); ee_home_ref(2)], t_home_ref);\n' ...
    '    z_ref_ts = timeseries([ee_home_ref(3); ee_home_ref(3)], t_home_ref);\n' ...
    'end\n'], ...
    escape_quotes(projectRoot), escape_quotes(analysisRoot), eeHome(1), eeHome(2), eeHome(3));
...
function set_switches_to_ui_source(subsystemPath)
switchNames = {'X Source', 'Y Source', 'Z Source'};
for idx = 1:numel(switchNames)
    switchBlock = [subsystemPath '/' switchNames{idx}];
    if getSimulinkBlockHandle(switchBlock) > 0
        set_param(switchBlock, 'sw', '0');
    end
end
end
...
script = sprintf([ ...
        ...
        '    lastTheta = delta_command_to_model(params, repmat(params.home_command_deg, 3, 1));\n' ...
        'end\n\n' ...
